  ...
  def __init__(self,
    ...
    with tf.compat.v1.variable_scope(scope, default_name="bert"):
      with tf.compat.v1.variable_scope("embeddings"):
        # Disconned embeddings from the Transformer  
         embedding_size=config.hidden_size
         self.embedding_output = init_rand_variable([batch_size,seq_length,embedding_size], precision=config.precision)
      ...
      with tf.compat.v1.variable_scope("encoder"):
        ...
        attention_mask = create_attention_mask_from_input_mask(
            input_ids, input_mask, config.precision)
...
def embedding_lookup(input_ids,
  ...
  embedding_table = tf.compat.v1.get_variable(
        ...
        initializer=create_initializer(initializer_range), dtype=precision)

  flat_input_ids = tf.reshape(input_ids, [-1])
  if use_one_hot_embeddings:
    one_hot_input_ids = tf.one_hot(flat_input_ids, depth=vocab_size, dtype=precision)
...
def embedding_postprocessor(input_tensor,
  ...
  if use_token_type:
    ...
    one_hot_ids = tf.one_hot(flat_token_type_ids, depth=token_type_vocab_size, dtype=precision)
...
def create_attention_mask_from_input_mask(from_tensor, to_mask, precision):
  ...
  to_mask = tf.cast(
        tf.reshape(to_mask, [batch_size, 1, to_seq_length]), dtype=precision)
  broadcast_ones = tf.ones(
        shape=[batch_size, from_seq_length, 1], dtype=precision)
